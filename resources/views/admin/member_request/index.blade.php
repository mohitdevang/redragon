...
<div id="page-wrapper">
    ...
    <!-- your form fields go here -->
                   ...
                   @if(isset($request))
                                            ...
                                            <tr>
                                               <td width="2%"><input type="checkbox" name="item[]" value="{{ $req->id }}" class="chkItems" onclick="checkUnCheckParent()">
</td>
                                                <td>{{$s}}</td>
        ...
        onchange="changerequestStatus(this.value, {{ $req->id }}, '{{ $user->unique_id }}')">
    <option value="approve" {{ $req->status == 'approve' ? 'selected' : '' }}>Approve</option>
    <option value="disapprove" {{ $req->status == 'disapprove' ? 'selected' : '' }}>Disapprove</option>
...
<script type="text/javascript">
  ...
  function changerequestStatus(status,pageID,member_id){
  ...
  $.ajax({
            ...
            type: 'POST',
            url:  '{{ route('admin.request.change.status') }}',
            data :{'_token': token,'status' : status,'id' : pageID,'no_of_pin' : '','member_id' : member_id},
